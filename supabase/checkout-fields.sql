-- Smoak Paris — email client + choix du paiement (CB / espèces) à la livraison
...
alter table orders add constraint orders_payment_method_check
  check (payment_method in ('cb', 'especes'));

-- Remplace create_order (9 arguments) par une version à 11 arguments qui
-- enregistre aussi l'email client et le moyen de paiement choisi.
drop function if exists create_order(
  text, text, text, text, numeric, jsonb, jsonb, double precision, double precision
);
...
begin
  insert into orders (
    ...
    total_price, delivery_lat, delivery_lng, payment_method
  )
  values (
    p_customer_name, p_customer_phone, p_customer_email, p_delivery_address, p_delivery_note,
    p_total_price, p_delivery_lat, p_delivery_lng, p_payment_method
  )
